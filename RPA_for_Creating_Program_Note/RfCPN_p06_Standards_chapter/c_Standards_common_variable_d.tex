...
\SetRow{unusingVariables}
\ttNum460 & （不使用） &\\
\ttNum461 & 工具{\ttfamily T31}（Tスロット）A側\dimple~深さ補正値（深さに$+$補正） & \\
\ttNum462 & 工具{\ttfamily T31}（Tスロット）C側\dimple~深さ補正値（深さに$+$補正） & \\
\ttNum463 & 工具{\ttfamily T31}（Tスロット）B側\dimple~深さ補正値（深さに$+$補正） & \\
\ttNum464 & 工具{\ttfamily T31}（Tスロット）D側\dimple~深さ補正値（深さに$+$補正） & \\
\SetRow{unusingVariables}
\ttNum465 & （不使用） &\\
...
\SetRow{unusingVariables}
\ttNum601 & （予備） &\\
\ttNum602 & タッチセンサープローブ測定時（\dimple 除く）の近付き量 & 10.0\\
