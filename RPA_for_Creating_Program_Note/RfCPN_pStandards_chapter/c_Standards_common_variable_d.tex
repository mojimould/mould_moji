...
ここでは\DMname で使用する\index{コモンへんすう@コモン変数}コモン変数について述べる。
...
\begin{twoCtable}{}
\ttNum183 & （不使用）\\\hline
\ttNum184 & 測定（通り芯・\dimple 除く）後 一時停止 (0:non-stop, 1: \verb|M00|) & 0\\\hline
\ttNum185 & （不使用）\\\hline
\ttNum186 & \dimple 測定後 一時停止 (0:non-stop, 1: \verb|M00|) & 0\\\hline
\ttNum187 & \dimple 加工後 一時停止 (0:non-stop, 1: \verb|M00|, 2:\verb|P90003|) & 0\\\hline
\ttNum188 & （不使用）\\\hline
...
\modcaptionof{table}{\ttNum400\,-\ttNum412：主に計測用（\dimple 除く）}
\begin{twoCtable}{}
...
%\clearpage
\modcaptionof{table}{\ttNum413\,-\ttNum430：主に加工用（\dimple 除く）}
\begin{twoCtable}{}
...
%% common variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\modcaptionof{table}{\ttNum450\,-\ttNum474：主に\dimple~加工用}
\begin{twoCtable}{}
\ttNum450 & （不使用）\\\hline
\ttNum451 & 工具\verb|T31|（Tスロット）A側\dimple~深さ補正値（深さに$+$補正） & 0.06\\\hline
\ttNum452 & 工具\verb|T31|（Tスロット）C側\dimple~深さ補正値（深さに$+$補正） & 0.03\\\hline
\ttNum453 & 工具\verb|T31|（Tスロット）B側\dimple~深さ補正値（深さに$+$補正） & 0.06\\\hline
\ttNum454 & 工具\verb|T31|（Tスロット）D側\dimple~深さ補正値（深さに$+$補正） & 0.03\\\hline
\ttNum455 & （不使用）\\\hline
\hline
\ttNum456 & 工具\verb|T32|（Tスロット）A側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum457 & 工具\verb|T32|（Tスロット）C側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum458 & 工具\verb|T32|（Tスロット）B側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum459 & 工具\verb|T32|（Tスロット）D側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum460 & （不使用）\\\hline
\hline
\ttNum461 & 工具\verb|T33|（Tスロット）A側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum462 & 工具\verb|T33|（Tスロット）C側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum463 & 工具\verb|T33|（Tスロット）B側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum464 & 工具\verb|T33|（Tスロット）D側\dimple~深さ補正値（深さに$+$補正）\\\hline
\ttNum465 & （不使用）\\\hline
& （以下予備）
\end{twoCtable}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\ttNum662 & （\dimple 加工用予備） & \\\hline
\ttNum663 & （\dimple 加工用予備） & \\\hline
& （以下予備）
\end{twoCtable}
...
%% common variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\modcaptionof{table}{\ttNum750\,-\ttNum774：\dimple~測定用 (\DMLthreeAC, \DMLthreeBD)}
\begin{twoCtable}{}
