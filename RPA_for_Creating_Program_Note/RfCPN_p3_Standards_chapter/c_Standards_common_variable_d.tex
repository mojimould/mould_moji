...
\begin{multicollongtblr}[white]{\ttNum175\,-\ttNum199：各工程後一時停止}{cX[l]c}
変数 & 内容 & 初期値\\
\SetRow{unusingVariables}
\ttNum175 & （以下 予備） &\\
\SetRow{unusingVariables}
...
\SetRow{unusingVariables}
\ttNum183 & （不使用） &\\
\ttNum184 & 芯出し測定後 一時停止 (0:non-stop, 1: {\ttfamily M00}) & 0\\
\SetRow{unusingVariables}
\ttNum185 & （不使用） &\\
\ttNum186 & \dimple 測定後 一時停止 (0:non-stop, 1: {\ttfamily M00}) & 0\\
\ttNum187 & \dimple 加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\SetRow{unusingVariables}
\ttNum188 & （不使用） &\\
\ttNum189 & トップ端面加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum190 & トップ外削加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum191 & トップ溝加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum192 & トップ外面取加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum193 & トップ内面取加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum194 & トップ座ぐり加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\SetRow{unusingVariables}
\ttNum195 & （不使用） &\\
\ttNum196 & ボトム端面加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum197 & ボトム外削加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum198 & ボトム外面取加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0\\
\ttNum199 & ボトム内面取加工後 一時停止 (0:non-stop, 1: {\ttfamily M00}, 2:{\ttfamily O900003}) & 0
\end{multicollongtblr}
...
\begin{multicollongtblr}[white]{\ttNum400\,-\ttNum404：初期設定}{cX[l]c}
変数 & 内容 & 初期値\\
\ttNum400 & トップ端面 全削り代 &\\
\ttNum401 & ボトム端面 全削り代(0 or \ttNum0: \ttNum400) &\\
\ttNum402 & 計測・加工 開始N番号 & 0\\
\ttNum403 & 通り芯測定(0:off, 1:on) & 0\\
\SetRow{unusingVariables}
\ttNum404 & （不使用） &\\
\end{multicollongtblr}

\begin{multicollongtblr}[white]{\ttNum405\,-\ttNum414：測定時の調整（\dimple 除く）}{cX[l]c}
変数 & 内容 & 初期値\\
\ttNum405 & {\ttfamily G54}$X$：ボトム外$X$芯出し（両側・片側測定）測定位置$Z-$補正（$X$自動補正） & 0\\
\ttNum406 & {\ttfamily G54}$Y$：ボトム外$Y$芯出し（両側測定）測定位置$Z-$補正 & 0\\
\ttNum407 & {\ttfamily G55}$X$：ボトム内$X$芯出し（両側測定）測定位置$Z-$補正（$X$自動補正） & 0\\
\ttNum408 & {\ttfamily G55}$Y$：ボトム内$Y$芯出し（両側測定）測定位置$Z-$補正 & 0\\
\SetRow{unusingVariables}
\ttNum409 & （不使用） &\\
\ttNum410 & {\ttfamily G56}$X$：トップ外$X$芯出し（両側・片側測定）測定位置$Z-$補正（$X$自動補正） & 0\\
\ttNum411 & {\ttfamily G56}$Y$：トップ外$Y$芯出し（両側測定）測定位置$Z-$補正 & 0\\
\ttNum412 & {\ttfamily G57}$X$：トップ内$X$芯出し（両側測定）測定位置$Z-$補正（$X$自動補正） & 0\\
\ttNum413 & {\ttfamily G57}$Y$：トップ内$Y$芯出し（両側測定）測定位置$Z-$補正 & 0\\
\SetRow{unusingVariables}
\ttNum414 & （不使用） &\\
\end{multicollongtblr}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\clearpage
\begin{multicollongtblr}[white]{\ttNum415\,-\ttNum424：加工時の調整（端面・\dimple 除く）}{cX[l]c}
変数 & 内容 & 初期値\\
\ttNum415 & トップ外削 A面肉厚$+$補正（外削中心$X-$補正） & 0\\
\ttNum416 & トップ外削 仕上げ前 一時停止 (0:non-stop, 1:{\ttfamily M00}, 2:扉前{\ttfamily M00}) & 0\\
\ttNum417 & トップ外削 仕上げ加工 追加回数 (上限3) & 0\\
\SetRow{unusingVariables}
\ttNum418 & （不使用） &\\
\ttNum419 & 溝位置$+$補正（溝幅不変） & 0\\
\ttNum420 & 溝幅$+$補正 & 0\\
\ttNum421 & 溝A面深さ$+$補正（溝径中心$X-$補正） & 0\\
\ttNum422 & 溝幅$Z$方向中央切削（3回加工）(0:off, 3:on) & 0\\
\ttNum423 & 溝 仕上げ前 一時停止 (0:non-stop, 1:{\ttfamily M00}, 2:扉前{\ttfamily M00}) & 0\\
...
\ttNum426 & トップ外面取 仕上げ前 一時停止 (0:non-stop, 1:{\ttfamily M00}, 2:扉前{\ttfamily M00}) & 0\\
\ttNum427 & トップ外面取 仕上げ加工 追加回数 (上限3) & 0\\
\SetRow{unusingVariables}
\ttNum428 & （不使用） &\\
\ttNum429 & トップ内面取$X+$補正 & 0\\
\ttNum430 & トップ内面取 仕上げ前 一時停止 (0:non-stop, 1:{\ttfamily M00}, 2:扉前{\ttfamily M00}) & 0\\
\ttNum431 & トップ内面取 仕上げ加工 追加回数 (上限3) & 0\\
\SetRow{unusingVariables}
\ttNum432 & （不使用） &\\
\SetRow{unusingVariables}
\ttNum433 & （座ぐり$X+$補正用 予備） &\\
\SetRow{unusingVariables}
\ttNum434 & （座ぐり 仕上げ前 一時停止用 予備） &\\
\SetRow{unusingVariables}
\ttNum435 & （座ぐり 仕上げ加工 追加回数用 予備） &\\
\SetRow{unusingVariables}
\ttNum436 & （不使用） &\\
\ttNum437 & ボトム外削 A面肉厚$+$補正（外削中心$X+$補正） & 0\\
\ttNum438 & ボトム外削 仕上げ前 一時停止 (0:non-stop, 1:{\ttfamily M00}, 2:扉前{\ttfamily M00}) & 0\\
\ttNum439 & ボトム外削 仕上げ加工 追加回数 (上限3) & 0\\
\SetRow{unusingVariables}
\ttNum440 & （不使用） &\\
\ttNum441 & ボトム外面取$X+$補正 & 0\\
\ttNum442 & ボトム外面取 仕上げ前 一時停止 (0:non-stop, 1:{\ttfamily M00}, 2:扉前{\ttfamily M00}) & 0\\
\ttNum443 & ボトム外面取 仕上げ加工 追加回数 (上限3) & 0\\
\SetRow{unusingVariables}
\ttNum444 & （不使用） &\\
\ttNum445 & ボトム内面取$X+$補正 & 0\\
\ttNum446 & ボトム内面取 仕上げ前 一時停止 (0:non-stop, 1:{\ttfamily M00}, 2:扉前{\ttfamily M00}) & 0\\
\ttNum447 & ボトム内面取 仕上げ加工 追加回数 (上限3) & 0\\
\SetRow{unusingVariables}
\ttNum448 & （不使用） &\\
\SetRow{unusingVariables}
\ttNum449 & （予備） &
...
変数 & 内容 & 初期値\\
\ttNum450 & 工具{\ttfamily T31}（Tスロット）A側\dimple~深さ補正値（深さに$+$補正） & 0.06\\
\ttNum451 & 工具{\ttfamily T31}（Tスロット）C側\dimple~深さ補正値（深さに$+$補正） & 0.03\\
\ttNum452 & 工具{\ttfamily T31}（Tスロット）B側\dimple~深さ補正値（深さに$+$補正） & 0.06\\
\ttNum453 & 工具{\ttfamily T31}（Tスロット）D側\dimple~深さ補正値（深さに$+$補正） & 0.03\\
\SetRow{unusingVariables}
\ttNum454 & （不使用） &\\
\ttNum455 & 工具{\ttfamily T32}（Tスロット）A側\dimple~深さ補正値（深さに$+$補正） &\\
\ttNum456 & 工具{\ttfamily T32}（Tスロット）C側\dimple~深さ補正値（深さに$+$補正） &\\
\ttNum457 & 工具{\ttfamily T32}（Tスロット）B側\dimple~深さ補正値（深さに$+$補正） &\\
\ttNum458 & 工具{\ttfamily T32}（Tスロット）D側\dimple~深さ補正値（深さに$+$補正） &\\
\SetRow{unusingVariables}
\ttNum459 & （不使用） &\\
\ttNum460 & 工具{\ttfamily T33}（Tスロット）A側\dimple~深さ補正値（深さに$+$補正） &\\
\ttNum461 & 工具{\ttfamily T33}（Tスロット）C側\dimple~深さ補正値（深さに$+$補正） &\\
\ttNum462 & 工具{\ttfamily T33}（Tスロット）B側\dimple~深さ補正値（深さに$+$補正） &\\
\ttNum463 & 工具{\ttfamily T33}（Tスロット）D側\dimple~深さ補正値（深さに$+$補正） &\\
\SetRow{unusingVariables}
\ttNum464 & （不使用） &\\
\SetRow{unusingVariables}
$\cdots$ & （以下予備） &
...
\ttNum631 & 外面取加工1回あたりの削り代（直径） & 2.0\\
\ttNum632 & 外面取加工 仕上げ前 残り削り代（直径） & 1.0\\
\ttNum633 & 内面取加工1回あたりの削り代（直径） & 2.0\\
...
変数 & 内容 & 設定値\\
\ttNum650 & 早送り$Z$：{\ttfamily T50}アプローチ以外 & 12000\\
...
%% subsection 18.6.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\ttNum700\,-\ttNum724：\dimple~サブプログラム\DLone 用}
...
%% subsection 18.6.2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\ttNum725\,-\ttNum749：\dimple~サブプログラム\DLtwoAC\DLtwoBD 用}
