...
ここでは主に、\index{たんめんそとがわRめんとり@端面外側R面取}\textbf{端面外側R面取}および\index{たんめんうちがわRめんとり@端面内側R面取}\textbf{端面内側R面取}に関する測定・加工に必要な\expandafterindex{きかがくてきせいしつ（たんめんRめんとり）@幾何学的性質（端面R面取）}幾何学的性質を考える。

\index{Rめんとりかこう@R面取加工}R面取加工では、\index{ボールエンドミル}ボールエンドミルの工具を用いて行われるのが一般的である。
ボールエンドミルは\DMname には搭載しない予定であり、基本的には端面R面取は\index{てもちけんまき@手持ち研磨機}手持ち研磨機を用いて\index{さぎょうしゃ@作業者}作業者の手作業で行う。
...
トップ側およびボトム側の\index{たんめんそとがわRめんとり@端面外側R面取}端面外側R面取の大きさを、それぞれ$r_\mathrm{To}$, $r_\mathrm{Bo}$とする。
