...
ここでは主に、\index{たんめんそとRめんとり@端面外R面取}\textbf{端面外R面取}および\index{たんめんうちRめんとり@端面内R面取}\textbf{端面内R面取}に関する測定・加工に必要な\expandafterindex{きかてきせいしつ（たんめんRめんとり）@幾何的性質（端面R面取）}幾何的性質を考える。

\index{たんめんRめんとりかこう@端面R面取加工}R面取加工では、\index{ボールエンドミル}ボールエンドミルの工具を用いて行われるのが一般的である。
...
\index{たんめんうちRめんとりちょう@端面内R面取長}端面内R面取長を$r_\mathrm{Ti}$, $r_\mathrm{Bi}$についても同様に、
\begin{align*}
