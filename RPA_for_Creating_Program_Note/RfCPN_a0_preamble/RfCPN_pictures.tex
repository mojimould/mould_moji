...
% 値の計算
\pgfmathsetmacro{\Ax}{11.0}      %A:T_iのx座標
\pgfmathsetmacro{\Ay}{3.5}       %A:T_iのy座標
\pgfmathsetmacro{\Bx}{2.0+(\Ax)} %B:T_oのx座標
\pgfmathsetmacro{\Cy}{-4.2}      %C:B_iのy座標
...
\pgfmathsetmacro{\Uw}{0.7} %Uw:受板の幅
\pgfmathsetmacro{\Ul}{0.65} %Ul:受板の長さ
\pgfmathsetmacro{\Ur}{0.55} %Uw:受板の半径
\pgfmathsetmacro{\Ub}{\Ri*sin(asin((\TUb-\Uw/2)/(\Ri-\Ur)))}     %Ub:受板-モールド接点のy座標
\pgfmathsetmacro{\Ux}{sqrt((\Ri)^2-(\Ub)^2)}  %TUx:受板-モールド接点のx座標
...
\draw[<->] ([xshift=-1.25mm]TBUex) -- ([xshift=-1.25mm]BUex) node[midway, left] {$\sigma$};
% 点を描画
\fill (Ut) circle (1pt);
\fill (Ub) circle (1pt);
\fill (TUc) circle (1pt);
...
\node at (Ut) [right] {U$_\mathrm T$};
\node at (Ub) [right] {U$_\mathrm B$};
\node at (TUc) [above] {U$_\mathrm c$};
\node at (BUc) [below] {U$_\mathrm c$};
\end{tikzpicture}%
}
