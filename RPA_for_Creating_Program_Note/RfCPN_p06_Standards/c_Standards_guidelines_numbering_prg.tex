...
%  \item\label{item:5Kaux} 加工の種類に依存しない加工のプログラム(\ttfamily{40\textbackslash d{4}})
  ...
  \item%\label{item:5KCO}
    主に、\expandafterindex{\yomiEndFaceOutChamfer かこう@\nameEndFaceOutChamfer 加工}\nameEndFaceOutChamfer 加工の位置決めを行うプログラム({\ttfamily 44\textbackslash d{4}})
  \item%\label{item:5KCI}
    主に、\index{\yomiEndFaceInChamfer かこう@\nameEndFaceInChamfer 加工}\EndFaceInChamfer 加工の位置決めを行うプログラム({\ttfamily 45\textbackslash d{4}})
%  \item\label{item:5KZ} \EndFaceBoring の加工のプログラム(\ttfamily{46\textbackslash d{4}})
  \setcounter{enumii}{8}
