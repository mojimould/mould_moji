...
\DimpleMilling における留意事項の1つに、モールドの内面（特にトップ端）と工具が接触してしまう\index{アンダーカット}アンダーカットというものがある
%% footnote %%%%%%%%%%%%%%%%%%%%%
...
\pageeqref{eq:tableTRc}より、テーブルを$-\theta$傾けて\AlocationLength の調整を行った場合、テーブル中心Pを原点とした\CenterCurvatureLine のトップ端における$X$座標は、
\begin{align*}
...
\end{array}
\end{align*}
ここで、\DimpleIRow の\CurvatureCenter と\expandafterindex{トップたんの\yomiCurvatureCenter@トップ端の\nameCurvatureCenter}トップ端の\nameCurvatureCenter との$X$座標の差を、
%% label{eq:}
\begin{align}
...
%% Column %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{Column}{\BfaceDimpleMilling, \DfaceDimpleMilling で考慮すべき点}
\paragraph*{工具径とシャンク径}
...
また\DimpleMilling は\index{トップたんのないけいちゅうしん@トップ端の内径中心}トップ端における内径中心を基準にして切削を行う。
...
たとえば\Dimple の場合、$g_\mathrm T$は通常（今の場合はテーブル中心Pより負側に\CurvatureCenter があることが多いので）負の値として得られることに注意。
...
%% footnote %%%%%%%%%%%%%%%%%%%%%
\footnote{ここではトップ端における\CurvatureCenter を、トップ端における内径中心と同一視している。}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{align*}
  ...
  \right].
\end{align*}
したがって、傾けた後のトップ端の\CurvatureCenter と$i$列目に対する\CurvatureCenter との差分は、
