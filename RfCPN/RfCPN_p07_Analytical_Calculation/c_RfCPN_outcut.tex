...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{hosoku}
\EndFaceInCChamfer がある場合、\OutcutAsideThickness$\tau_\mathrm T$, $\tau_\mathrm B$は、\EndFace から\index{Cめんとりちょう@C面取長}C面取長$c_\mathrm{Ti}$または$c_\mathrm{Bi}$の位置における\index{すんぽう（\yomiThickness）@寸法（\nameThickness）}寸法が与えられる。
...
\expandafterindex{\yomiTopOutcut@\nameTopOutcut}トップ側にも外削がある場合、\BottomOutcut から\CenterlineEndFaceDif を指定する形で\TopOutcutCenter の位置を決めるのが通常である。
...
\index{テーブルちゅうしん@テーブル中心}テーブル中心Pを\index{げんてんP@原点P}原点とした場合の、\index{\yomiTopOutcutCenter@\nameTopOutcutCenter}トップ側の外削中心$\mathfrak T_\mathrm c'$のおおよその$X$座標は、\pageeqref{eq:spacerTc}より、
\begin{align*}
...
\end{alignat*}
\end{subequations}
\BottomOutcutLength を$h_\mathrm B$とすると、\expandafterindex{かたむき（\yomiOutcut）@傾き（\nameOutcut）}\nameOutcut 部分の傾きはボトム端から距離$\nicefrac{h_\mathrm B}2$の断面に平行になる形にとるので、そのときの\expandafterindex{かたむきかく（\yomiOutcut）@傾き角（\nameOutcut）}傾き角$\theta_b$は、
\begin{align*}
