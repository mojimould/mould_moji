...
\modHeadchapter{計算の必要な数値情報（\CreatedNCSubPrg 用）\TBW}
ここでは主に\CreatedNCSubPrg の記述に際して、\index{すうちけいさん@数値計算}数値計算に必要または有用な部分をピックアップする。
...
\TableCenter\expandafterindex{P（\yomiTableCenter)@P（\nameTableCenter）}Pを\index{げんてんP@原点P}原点とした\BottomOutcutCenter$\mathfrak B_\mathrm c'$の（おおよその）$X$座標は、\pageeqref{eq:gaisakucenterBt}より、
\begin{align*}
  \HLbox{%
    \Delta_x'\cos\theta
