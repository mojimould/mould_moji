...
\item[{\sarrow[red]}]\index{オペレータ}オペレータに対してNCメインプログラムの直接編集を強いている状態になっている
\item[{\sarrow[red]}]NCプログラム専門の担当者が存在せず、オペレータが兼任している状態になっている
\end{enumerate}
\end{Issues}
...
\item[{\sarrow[red]}]ジグ・ワークの寸法から機械的に見出だせるにかかわらず、放置され続けている
\end{enumerate}
\end{Issues}
...
\begin{Issues}{根本的な問題}
\begin{enumerate}[label=\sarrow]
\item[{\sarrow[red]}] \expandafterindex{\yomiDrawing（モールド）@\nameDrawing（モールド）}\nameDrawing と\index{NCプログラム}NCプログラムの作成作業の工程が直列になっている
\item[{\sarrow[red]}] \expandafterindex{\yomiDrawing のさくせい）@\nameDrawing の作成}\nameDrawing の作成が、\index{CADソフトウェア}CADソフトウェアを用いて手動でなされている
