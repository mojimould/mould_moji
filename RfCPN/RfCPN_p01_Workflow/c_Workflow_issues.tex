...
\item[{\sarrow[red]}]仕様の変更により容易に改善が可能にも関わらず、事態が放置され続けている
\end{enumerate}
\end{Issues}
...
\item[{\sarrow[red]}]\EndFaceChamferMilling に関する解析的な幾何情報を導出しないまま放置し続けている
\end{enumerate}
\end{Issues}
...
\begin{Issues}{\EndFaceOutCChamferMilling の位置調整\TBW}
\begin{enumerate}[label=\sarrow]
\item[{\sarrow[red]}]
\end{enumerate}
\end{Issues}
...
\begin{Issues}{\EndFaceInCChamferMilling の位置調整\TBW}
\begin{enumerate}[label=\sarrow]
\item[{\sarrow[red]}]
\end{enumerate}
\end{Issues}
...
%% subsection 02.03.02 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\index{NCプログラム}NCプログラムの作成における作業効率\TBW}

\begin{Issues}{\KeywayCenter 位置の手動計算による導出\TBW}
\begin{enumerate}[label=\sarrow]
\item[{\sarrow[red]}]\KeywayCenter の
\end{enumerate}
\end{Issues}
...
\begin{enumerate}[label=\sarrow]
\item[{\sarrow[red]}]\index{ワークざひょうけいげんてん@ワーク座標系原点}ワーク座標系原点の概算値を見出すために、オペレータに直接機内で測定するよう強要している
\item[{\sarrow[red]}]ジグ・ワークの寸法から計算で容易に見出だせるにかかわらず、放置され続けている
\end{enumerate}
\end{Issues}
