...
%% section 1.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\modHeadsection{\MMC における工程および使用ツール}
...
測定 & 加工\\
端面外側AC方向 芯出し（外側 両側$X$測定） & \EndFacecutMilling\\
端面外側BD方向 芯出し（外側 両側$Y$測定） & \OutcutMilling\\
\OutcutWidth AC方向 芯出し（内側 片側$X$測定） & \KeywayMilling\\
端面内側AC方向 芯出し（内側 両側$X$測定） & \EndFaceOutCChamferMilling\\
端面内側BD方向 芯出し（内側 両側$Y$測定） & \EndFaceInCChamferMilling\\
...
\item \index{がいけい@外径}外径・\index{\yomiThickness（たんめん）@\nameThickness（端面）}端面部の\nameThickness・\index{たんめんそとがわコーナーR@端面外側コーナーR}端面外側コーナーRを確認し、それに応じて\index{こうぐけいほせいち@工具径補正値}工具径補正値を決定する
...
\item \index{ないけい（たんめん）@内径（端面）}端面の内径・\OutcutAsideThickness・内面の\PlatingThk から、\OutcutCenter$X$位置用のパラメータを手動による計算で決定する
\item \CurvedOutcut の場合は、\index{かたむきかく（\yomiCurvedOutcut）@傾き角（\nameCurvedOutcut）}傾き角を手動による計算で決定する
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PARAMETER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% subsection 01.2.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\EndFaceOutCChamfer 加工中}
\begin{enumerate}[label=\sarrow]
...
%% subsection 01.2.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\EndFaceInCChamfer 加工中}
\begin{enumerate}[label=\sarrow]
