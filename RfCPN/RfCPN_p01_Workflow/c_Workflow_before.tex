...
測定 & 加工\\
端面外側AC方向 芯出し（外側 両側$X$測定） & \index{たんめんさくかこう@端面削加工}端面削加工\\
...
端面内側AC方向 芯出し（内側 両側$X$測定） & \expandafterindex{\yomiEndFaceOutChamfer かこう@\nameEndFaceOutChamfer 加工}\nameEndFaceOutChamfer 加工\\
端面内側BD方向 芯出し（内側 両側$Y$測定） & \index{\yomiEndFaceInChamfer かこう@\nameEndFaceInChamfer 加工}\nameEndFaceInChamfer 加工\\
\CenterlineEndFaceDifAC 測定（外側 片側$Z$測定） & \expandafterindex{\yomiEndFaceBoring かこう@\nameEndFaceBoring 加工}\nameEndFaceBoring 加工\\
\CenterlineEndFaceDifBD 測定（外側 片側$Y$測定） & \index{インローかこう@インロー加工}インロー加工\\
...
\MMC による加工については、基本的に\MMC 本体の操作画面を用いてNCプログラム等の作成・編集やパラメータの変更等は人手で直接行う。
測定は主に\index{みつびしマシニングセンタむじんかシステム@三菱マシニングセンタ無人化システム}三菱マシニングセンタ無人化システムを用いる。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{端面削部分}
\index{たんめんさくかこう@端面削加工}端面削加工については、全明細に共通の形で存在する。

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% subsubsection 01.1.2.2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{外削部分}
\index{がいさくかこう@外削加工}外削加工については、明細により\index{がいさくのうむ@外削の有無}外削の有無または\index{がいさくのけいじょう@外削の種類}種類の違いが存在する。
\begin{enumerate}[label=\sarrow]
...
\expandafterindex{\yomiKeyway かこう@\nameKeyway 加工}\Keyway 加工については、全明細のトップ側に存在し、明細により種類の違いが存在する。
\begin{enumerate}[label=\sarrow]
...
\index{たんめんめんとりかこう@端面面取加工}端面面取加工については、全明細に存在し、明細により種類の違いが存在する。
\begin{enumerate}[label=\sarrow]
\item 種類が\index{Cめんとり@C面取}C面取であれば、その\index{Cめんとりちょう@C面取長}C面取長により\index{マシニングセンタ}マシニングセンタによる加工を行うか判断を行う
\item \EndFaceChamferAngle を確認し、使用する\index{こうぐ（\yomiEndFaceChamfer）@工具（\nameEndFaceChamfer）}工具を決定する
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PARAMETER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{Parameter}{必要なパラメータ}
\PMChamferType%
\PMTopEndFaceOutChamferLength
\PMTopEndFaceOutChamferAngle%
\PMTopOutcutExists\\
\PMBottomEndFaceOutChamferLength%
\PMBottomEndFaceOutChamferAngle%
\PMBottomOutcutExists%
\end{Parameter}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{端面面取における寸法}
\begin{enumerate}[label=\sarrow]
\item \EndFaceOutChamfer の場合は、\index{がいさくのうむ@外削の有無}外削の有無を確認し、加工の径を決定する
\item \index{Cめんとり（がいさくようこうぐせんたん）@C面取（外削用工具先端）}外削用工具先端部がC面取の場合は、\index{がいさくのけいじょう@外削の形状}外削の形状を確認し、\index{こうぐ（がいさく）@工具（外削）}工具を決定する
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PARAMETER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{Parameter}{必要なパラメータ}
\paragraph*{トップ\EndFaceOutChamfer：外削のない場合}
\PMACOD
\PMBDOD
\PMTopEndFaceOutChamferLength
\PMODCornerR
\tcbline*
\paragraph*{トップ\EndFaceOutChamfer：外削のある場合}
\PMTopOutcutAC
\PMTopOutcutBD
\PMTopOutcutCornerR
\PMTopEndFaceOutChamferLength
\tcbline*
\paragraph*{ボトム\EndFaceOutChamfer：外削のない場合}
\PMACOD
\PMBDOD
\PMBottomEndFaceOutChamferLength
\PMODCornerR
\tcbline*
\paragraph*{ボトム\EndFaceOutChamfer：外削のある場合}
\PMBottomOutcutAC
\PMBottomOutcutBD
\PMBottomOutcutConerR
\PMBottomEndFaceOutChamferLength
\tcbline*
\paragraph*{トップ\EndFaceInChamfer}
\PMTopEndACID
\PMTopEndBDID
\PMTopEndIDCornerR\\
\PMTopEndFaceInChamferLength
\PMPlatingThk
\tcbline*
\paragraph*{ボトム\EndFaceInChamfer}
\PMBottomEndACID
\PMBottomEndBDID
\PMBottomEndIDCornerR\\
\PMBottomEndFaceInChamferLength
\PMPlatingThk
\end{Parameter}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item 測定した張出長から、\index{たんめんさくかこう@端面削加工}端面削加工における\index{ぜんけずりしろ（たんめんさく）@全削り代（端面削）}全削り代を手動でおおまかに計算する
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PARAMETER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{enumerate}[label=\sarrow]
\item トップ側およびボトム側の\index{ぜんけずりしろ（たんめんさく）@全削り代（端面削）}全削り代に応じて、\index{かこうかいすう（たんめんさくかこう）@加工回数（端面削加工）}端面削加工の回数を設定する
...
\item \CenterlineEndFaceDif がある場合\expandafterindex{テーブルのかいてんちゅうしん（\yomiMMC）@テーブルの回転中心（\nameMMC）}テーブルの回転中心とのずれを考慮して、\index{がいさくちゅうしん@外削中心}の$X$方向の長さを調整する
\end{enumerate}
これらの設定は、\MMC の操作画面から\index{メインプログラム}メインプログラムを直接手動で編集する形で行われる。
...
\subsection{端面削加工中}
\begin{enumerate}[label=\sarrow]
\item 必要に応じて、\index{1かいあたりのけずりしろ（たんめんさく）@1回あたりの削り代（端面削）}1回あたりの削り代を調整する
\end{enumerate}
...
\subsection{外削加工中}
\begin{enumerate}[label=\sarrow]
\item 必要に応じて\index{しあげかこう（がいさく）@仕上げ加工（外削）}仕上加工前に\index{いちじていし（NCプログラム）@一時停止（NCプログラム）}一時停止を行い、\index{Aがわにくあつ@A側肉厚}A側肉厚および\index{がいさくけい@外削径}外削径の測定を行う
...
%% subsection 01.2.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\EndFaceOutChamfer 加工中}
\begin{enumerate}[label=\sarrow]
\item 必要に応じて\expandafterindex{しあげかこう（\yomiEndFaceOutChamfer）@仕上げ加工（\nameEndFaceOutChamfer）}仕上加工前に\index{いちじていし（NCプログラム）@一時停止（NCプログラム）}一時停止を行い、\EndFaceOutChamfer の測定・位置の確認を行う
\item \EndFaceOutChamfer の位置を調整する場合は、該当する加工部分のパラメータをマシニングセンタの操作画面からメインプログラムを直接手動で編集する
\item \expandafterindex{かこうかいすう（\yomiEndFaceOutChamfer）@加工回数（\nameEndFaceOutChamfer）}加工の回数を変更する場合は、該当する加工部分をマシニングセンタの操作画面からメインプログラムを直接手動で編集する
\end{enumerate}
...
%% subsection 01.2.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\EndFaceInChamfer 加工中}
\begin{enumerate}[label=\sarrow]
\item 必要に応じて\expandafterindex{しあげかこう（\yomiEndFaceInChamfer）@仕上げ加工（\nameEndFaceInChamfer）}仕上加工前に\index{いちじていし@一時停止}一時停止を行い、\EndFaceInChamfer の状態の確認を行う
\item \EndFaceInChamfer の位置を調整する場合は、該当する加工部分のパラメータをマシニングセンタの操作画面からメインプログラムを直接手動で編集する
\item \expandafterindex{かこうかいすう（\yomiEndFaceInChamfer）@加工回数（\nameEndFaceInChamfer）}加工の回数を変更する場合は、該当する加工部分をマシニングセンタの操作画面からメインプログラムを直接手動で編集する
\end{enumerate}
...
\begin{enumerate}[label=\sarrow]
\item 所定の寸法の\index{たんめんめんとり@端面面取}端面面取を、\index{てもちけんまき@手持ち研磨機}手持ち研磨機を用いて手動で行う
\item \index{ばり}ばり等を除去を、\index{やすり}やすりを用いて全体的に手動で行う
\item \index{Cがわにくあつ（ボトムたんめん）@C側肉厚（ボトム端面）}ボトム端面のC側肉厚に応じて\index{こくいん@刻印}刻印の大きさを決定する
