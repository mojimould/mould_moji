...
\ttNum422 & \KeywayDiameter（直径）$+$補正（\AsideKeywayDepth 不変） & 0\\
...
\SetRow{unusingVariables}
\ttNum460 & （不使用） &\\
\ttNum461 & 工具{\ttfamily T31}（Tスロット）\AfaceDimple~深さ補正値（深さに$+$補正） & -0.035\\
\ttNum462 & 工具{\ttfamily T31}（Tスロット）\CfaceDimple~深さ補正値（深さに$+$補正） & -0.035\\
\ttNum463 & 工具{\ttfamily T31}（Tスロット）\BfaceDimple~深さ補正値（深さに$+$補正） & -0.005\\
\ttNum464 & 工具{\ttfamily T31}（Tスロット）\DfaceDimple~深さ補正値（深さに$+$補正） & -0.065\\
\SetRow{unusingVariables}
\ttNum465 & （不使用） &\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{marker}
\DimpleMilling 用工具の\expandafterindex{こうぐけいほせいち（\yomiDimpleMilling）@工具径補正値（\nameDimpleMilling）}工具径補正値または\expandafterindex{まもうりょう（\yomiDimpleMilling）@摩耗量（\nameDimpleMilling）}摩耗量を$\pm$変更すると、\DimpleDepth が全体的に$\mp$となる。
\end{marker}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\SetRow{unusingVariables}
\ttNum482 & （予備） &\\
\ttNum483 & \IncutBoring$X+$補正 & 0\\
\ttNum484 & \IncutBoring 径（直径）$+$補正 & 0\\
\ttNum485 & \IncutBoring{} 仕上げ前 一時停止 (0:{\ttfamily M00}, 1:non-stop, 2:扉前{\ttfamily M00}) & 1\\
\ttNum486 & \IncutBoring{} 仕上げ後 一時停止 (0:{\ttfamily M00}, 1:non-stop, 2:扉前{\ttfamily M00}) & 1\\
\SetRow{unusingVariables}
\ttNum487 & （予備） &\\
\SetRow{unusingVariables}
\ttNum488 & （予備） &\\
...
\SetRow{unusingVariables}
\ttNum607 & （予備） &\\
\ttNum608 & \EndFacecutMilling 用 内径輪郭径$-$補正量 & 4.0\\
\SetRow{unusingVariables}
\ttNum609 & （予備） &\\
\SetRow{unusingVariables}
...
%% section 19.7 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\modHeadsection{\ttNum900000\,-\ttNum900500：実測値}
...
%% subsection 19.7.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\ttNum900001\,-\ttNum900100：\Dimple 以外}
...
%\clearpage
\begin{multicollongtblr}[white]{\ttNum900038\,-\ttNum900049：\CurvedOutcutMilling 用 \KCurvedGaisakuRLeft}{cX[l]}
変数 & 内容\\
...
\SetRow{unusingVariables}
\ttNum900040 & （予備）\\
\ttNum900041 & 傾き後 \TopCurvedOutcut{} A側端点 $X$座標{} 計算値\pageeqref{eq:topcurvedoutcutAsideendpoint}\\
\ttNum900042 & 傾き後 \TopCurvedOutcut{} A側端点 $Z$座標{} 計算値\pageeqref{eq:topcurvedoutcutAsideendpoint}\\
\ttNum900043 & 傾き後 \TopCurvedOutcut{} C側端点 $X$座標{} 計算値\pageeqref{eq:topcurvedoutcutCsideendpoint}\\
\ttNum900044 & 傾き後 \TopCurvedOutcut{} C側端点 $Z$座標{} 計算値\pageeqref{eq:topcurvedoutcutCsideendpoint}\\
\SetRow{unusingVariables}
\ttNum900045 & （予備）\\
\ttNum900046 & 傾き後 \BottomCurvedOutcut{} A側端点 $X$座標{} 計算値\pageeqref{eq:bottomcurvedoutcutAsideendpoint}\\
\ttNum900047 & 傾き後 \BottomCurvedOutcut{} A側端点 $Z$座標{} 計算値\pageeqref{eq:bottomcurvedoutcutAsideendpoint}\\
\ttNum900048 & 傾き後 \BottomCurvedOutcut{} C側端点 $X$座標{} 計算値\pageeqref{eq:bottomcurvedoutcutCsideendpoint}\\
\ttNum900049 & 傾き後 \BottomCurvedOutcut{} C側端点 $Z$座標{} 計算値\pageeqref{eq:bottomcurvedoutcutCsideendpoint}\\
\end{multicollongtblr}
...
%% section 11.8 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\modHeadsection{\ttNum901000\,-\ttNum901024：\Palette・\Jig}
...
\ttNum901008 & \NoTwoPalette{} \JigCenter 機械座標$B$ & -0.013\\
\ttNum901009 & 工具中心機械座標$C$ & 0\\
\SetRow{unusingVariables}
\ttNum901010 & （予備） &\\
\ttNum901011 & \NoOnePalette{} \JigLength$2l$（機械座標$B$0 $Z$方向） & 660.0\\
\ttNum901012 & \NoOnePalette{} ジグ内側幅（機械座標$B$0 $Z$方向） & 410.0\\
\ttNum901013 & \NoOnePalette{} ジグ幅（機械座標$B$0 $X$方向） & 455.0\\
\SetRow{unusingVariables}
\ttNum901014 & \NoOnePalette{} ジグ底面-固定用ボルト取付具間距離（高さ, $Y$方向） &\\
\SetRow{unusingVariables}
\ttNum901015 & \NoOnePalette{} ジグ底面-固定用ボルト取付具間距離（幅, 機械座標$B$0 $X$方向） &\\
\SetRow{unusingVariables}
\ttNum901016 & （予備） &\\
\ttNum901017 & \NoTwoPalette{} \JigLength$2l$（機械座標$B$0における$Z$方向） & 660.0\\
\ttNum901018 & \NoTwoPalette{} ジグ内側幅（機械座標$B$0における$Z$方向） & 410.0\\
\ttNum901019 & \NoTwoPalette{} ジグ幅（機械座標$B$0における$X$方向） & 455.0\\
\ttNum901020 & \NoTwoPalette{} ジグ底面-固定用ボルト取付具間距離（高さ） & 257.0\\
\ttNum901021 & \NoTwoPalette{} ジグ底面-固定用ボルト取付具間距離（幅） & 247.0\\
\SetRow{unusingVariables}
\ttNum901022 & （予備） &\\
\SetRow{unusingVariables}
\ttNum901023 & （予備） &\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{hosoku}
その他の（\expandafterindex{ずめん（\yomiJig）@図面（\nameJig）}図面上の）\index{すんぽう（\yomiJig）@寸法（\nameJig）}寸法として、
\begin{enumerate}[label=\sarrow]
...
\ttNum901105 & 工具{\ttfamily T06}（サイドカッター）厚さ$t$ & 6.87\\
\SetRow{unusingVariables}
