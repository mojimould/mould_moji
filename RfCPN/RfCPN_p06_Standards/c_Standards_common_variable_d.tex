...
\ttNum187 & \Incut{} 仕上げ加工 追加回数 (上限3) & 0\\
\SetRow{unusingVariables}
\ttNum188 & （不使用） &\\
...
\SetRow{unusingVariables}
\ttNum482 & （予備） &\\
\ttNum483 & \Incut$X+$補正 & 0\\
\ttNum484 & \Incut 径（直径）$+$補正 & 0\\
\ttNum485 & \Incut{} 仕上げ前 一時停止 (0:{\ttfamily M00}, 1:non-stop, 2:扉前{\ttfamily M00}) & 1\\
\ttNum486 & \Incut{} 仕上げ後 一時停止 (0:{\ttfamily M00}, 1:non-stop, 2:扉前{\ttfamily M00}) & 1\\
\SetRow{unusingVariables}
\ttNum487 & （予備） &\\
\SetRow{unusingVariables}
\ttNum488 & （予備） &\\
...
\SetRow{unusingVariables}
\ttNum601 & （予備） &\\
\ttNum602 & タッチセンサープローブ測定時（\Dimple 除く）の近付き量 & 7.0\\
...
\SetRow{unusingVariables}
\ttNum619 & （予備） &\\
\ttNum620 & \DimpleMeasurement 時 近付き量 & 4.0 \\
...
\ttNum637 & \IncutMilling1回あたりの削り代 & 2.0\\
\ttNum638 & \IncutMilling{} 仕上げ前 残り削り代 & 1.0\\
\SetRow{unusingVariables}
\ttNum639 & （予備） & \\
\SetRow{unusingVariables}
\ttNum640 & （予備） & \\
...
\ttNum670 & \IncutMilling：直線 & \IncutLinearFeedRate\\
\ttNum671 & \IncutMilling：コーナー & \IncutCornerFeedRate\\
...
\ttNum681 & \IncutMilling & \IncutSpindleSpeed \\
...
\SetRow{unusingVariables}
\ttNum901000 & （予備） &\\
\ttNum901001 & パレット\ttNum1 \JigCenter 機械座標$X$ & -550.069\\
...
\ttNum901008 & パレット\ttNum2 \JigCenter 機械座標$B$ & -0.013\\
