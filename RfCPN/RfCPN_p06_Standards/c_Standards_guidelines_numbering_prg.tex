...
%  \item\label{item:5Kaux} 加工の種類に依存しない加工のNCプログラム(\ttfamily{40\textbackslash d{4}})
  \item%\label{item:5KF}
    主に、\EndFacecutMilling の位置決めを行うNCプログラム({\ttfamily 41\textbackslash d{4}})
  \item\label{item:5KO} 主に、\OutcutMilling の位置決めを行うNCプログラム({\ttfamily 42\textbackslash d{4}})
  \item%\label{item:5KK}
    主に、\KeywayMilling の位置決めを行うNCプログラム({\ttfamily 43\textbackslash d{4}})
  \item%\label{item:5KCO}
    主に、\EndFaceOutCChamferMilling の位置決めを行うNCプログラム({\ttfamily 44\textbackslash d{4}})
  \item%\label{item:5KCI}
    主に、\EndFaceInCChamferMilling の位置決めを行うNCプログラム({\ttfamily 45\textbackslash d{4}})
%  \item\label{item:5KZ} \EndFaceBoringMilling のNCプログラム(\ttfamily{46\textbackslash d{4}})
  \setcounter{enumii}{8}
...
\item 6桁目が\ref{item:6Kmain}（\Dimple 以外の加工）の場合、4桁目を以下にように分類する
  ...
  \item 5桁目が\ref{item:5KO}（\OutcutMilling）の場合、4桁目を以下のように分類する
    ...
    \item 端面に垂直方向の\OutcutMilling のNCプログラム({\ttfamily{420\textbackslash d{3}}})
