...
\input{preamble_Mould_Analytical_Calculation_Note/style_Mould_Analytical_Calculation_Note}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ボトム側外削径の中心$\mathfrak B_\text c'$の（おおよその）X座標は、\pageeqref{eq:spacerBc}より、
\begin{align*}
...
テーブルを$-\theta$傾けた場合、テーブル中心Pを原点としたボトム側外削径の中心$\mathfrak B_\text c'$の（おおよその）X座標は、\pageeqref{eq:tableBc}より、
\begin{align*}
  \varDelta'\cos\theta-\frac{\sqrt{R_\text o^2-f_\text B^2}+\sqrt{R_\text i^2-f_\text B^2}}2
  -\frac{w_\text B}2-\tau_\text B+\frac{\mathfrak W_\text B}2\ .
\end{align*}
...
テーブル中心Pを原点とした場合の、トップ側外削径の中心$\mathfrak T_\text c'$のおおよそのX座標は、\pageeqref{eq:spacerTc}より、
\begin{align*}
...
テーブルを$-\theta$傾けた場合、テーブル中心Pを原点としたボトム側外削径の中心$\mathfrak T_\text c'$の（おおよその）X座標は、\pageeqref{eq:tableTc}より、
\begin{align*}
  \frac{\sqrt{R_\text o^2-f_\text T^2}+\sqrt{R_\text i^2-f_\text T^2}}2-\varDelta'\cos\theta
  +\frac{w_\text T}2+\tau_\text T-\frac{\mathfrak W_\text T}2\ .
\end{align*}
...
\end{enumerate}
\end{tcolorbox}\noindent
この２点を通る直線と鉛直方向との角度を、傾き角$-\phi$ ($0 < \phi < \nicefrac\pi2$)とする
%% footnote %%%%%%%%%%%%%%%%%%%%%
\footnote{振分長の調整に用いたテーブルの傾き角$\theta$と混同しないように注意。}。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
なお、トップ端のAC内径は$w_{\text A0}$で代用してもよいものとする。

BおよびD面の内面溝についても（特にA側内面に対して）アンダーカットを避ける必要がある。
しかし、傾けなくてもアンダーカットが発生せず加工できる場合もある。
そのためB, D面の内面溝については、A側内面のトップ端のX座標と、それと最も$X$座標が近い内面溝との（$X$方向の）距離を算出する。
そしてそれを工具径と比べることで、傾けるかどうかを決めることにする。
また傾ける場合は、上記の角度$\phi$を用いることにする。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% hosoku %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{hosokubox}
C側の内面溝については、そもそもアンダーカットが生じることがない（湾曲が外側に向いている）ので傾ける必要がない。
\end{hosokubox}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{align*}
で与えられる。
これより、A側内面溝に対する傾き角$\phi_\text A$ ($\phi_\text A \geqq 0$)は、
\begin{align}
  \label{eq:dKatamuki}
  \tan\phi
  = \frac{\displaystyle
          \sqrt{\left(R_\text c+\frac{w'_{\text Aq}}2\right)^{\!\!2}-(f_\text T-q)^2}
          -\sqrt{\left(R_\text c+\frac{w'_{\text A0}}2\right)^{\!\!2}-f_\text T^2}}q
\end{align}
で与えられる。
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Column %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
C側内面溝については前述の通り傾ける必要はない。
しかし、傾けたまま加工をすると形状が歪になってしまうため、内面溝の形状をよりよくするためには傾いていないほうが望ましい。
そうした事情も考慮する場合は、C面に対しても傾けて加工をする必要が出てくる。
そのためC側内面溝に対する傾き角$\phi_\text C$についてもここで与えておく。
なお、$\phi_\text C$は上記の（A面に対する）$\phi$とは異なるものとしている。
具体的には以下の２点を基準として角度$\phi_\text C$を取ることとする。
\begin{enumerate}
\item[a)]
C側内面の内面溝１列目（トップ端から$q$）の位置
\item[b)]
C側内面の内面溝１列目（トップ端から$q+(m-1)p_z$）の位置
\end{enumerate}
\tcbline
...
これらのX座標は\pageeqref{eq:dPosXBefore}よりそれぞれ、
\begin{align*}
  m-1\text{列目：}&\quad
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{傾き後の内面溝}
機内での回転は、テーブル中心Oを原点として行われ、また内面溝の加工はトップ端における内径中心を基準にして切削を行う。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
このときこれの$XZ$成分[$g_{tx}$, $f_t'$]について、$XZ$平面をテーブル中心Oを原点とした複素平面として表すと、
\begin{align*}
  \sqrt{g_{tx}^2+f_t'^2}e^{i}
\end{align*}


角度$-\phi$だけ傾けた後のトップ端内面中心の座標$g'_t$は
%% footnote %%%%%%%%%%%%%%%%%%%%%
...
%% subsection 6.4.1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{傾き後の内面溝（A面側）}
A面側の内面溝については、テーブル中心Oを中心に$-\phi$だけ傾けて加工を行う。
$\phi$は\pageeqref{eq:dKatamuki}で与えられる。
このとき、傾けた後の$i$列目$j$番目の内面溝の位置は、\pageeqref{eq:dPosXBefore}, \pageeqref{eq:dPosYBefore}, \pageeqref{eq:dPosZBefore}より、
\begin{align*}
  \left[
  \begin{array}{c}
...
%% subsection 6.3.2 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{傾き後の内面溝（C面側）}
C面側の内面溝については傾けずに行うため、そのままの座標を用いればよい。
よってC面側に対する$i$列目$j$番目の内面溝の位置は、
\begin{align*}
  \left[
  \begin{array}{c}
...
\begin{align*}
  \left[
  \begin{array}{c}
    \mathcal D_{x(i+1),\text C}-\mathcal D_{xi,\text C}\\
    \mathcal D_{y(i+1)j,\text C}-\mathcal D_{yij,\text C}\\
    \mathcal D_{z(i+1)}-\mathcal D_{zi}
  \end{array}
  \right]
  = \left[
    \begin{array}{c}
      \displaystyle
      \mathcal L_{i+1, i}-\frac{w'_{\text Aq+ip_z}-w'_{\text Aq+(i-1)p_z}}2\\[8pt]
      \displaystyle-\frac{d_{i+1}-d_i}2\\[5pt]
      \displaystyle
      -p_z
    \end{array}
    \right]\ .
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{傾けない場合}
B, D面側について、傾けずに加工を行う場合はそのままの座標を用いればよい。
よってB, D面側に対する$i$列目$j$番目の内面溝の位置は、それぞれ、
\begin{align*}
  \left[
  \begin{array}{c}
    \mathcal D_{xij,\text B}'\\
...
\begin{align*}
  \left[
  \begin{array}{c}
    \mathcal D_{xi(j+1),\text B}-\mathcal D_{xij,\text B}\\
    0\\
    0
  \end{array}
  \right]
...
\begin{align*}
  \left[
  \begin{array}{c}
    \mathcal D_{x(i+1)j,\text B}-\mathcal D_{xij,\text B}\\
  ...
  = \left[
    \begin{array}{c}
      \displaystyle\mathcal L_{i+1, i}+\frac{d_{i+1}-d_i}2\\[6pt]
      \displaystyle\frac{w'_{\text Bq+ip_z}-w'_{\text Bq+(i-1)p_z}}2\\[6pt]
      -p_z
    \end{array}
    \right]\ .
\end{align*}
D面については、これの$Y$成分の符号を反対にしたものである。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{傾ける場合}
傾けて加工を行う場合は、$-\phi$だけ傾けて行う。
したがって傾けた後のB面側に対する$i$列目$j$番目の内面溝の位置は、A面側のときと同様に、
\begin{align*}
  \left[
  \begin{array}{c}
    \mathcal D_{xij,\text B}'\\
...
\begin{align*}
 ...
 &= \left[
    ...
    \right]\ .
\end{align*}
D面に対しては、これの$Y$成分の符号を入れ換えたものとなる。
...
%% Column %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{Column}{傾き後の内面溝（C面側）}
C面側を傾ける場合、傾けた後の$i$列目$j$番目の内面溝の位置は$\phi_\text C$を用いて以下のように表すことができる。
\begin{align*}
  \left[
  \begin{array}{c}
    \mathcal D_{xij,\text C}'\\
  ...
  = \left[
    \begin{array}{c}
      \mathcal D_{xij,\text C}\cos\phi_\text C+\mathcal D_{zi}\sin\phi_\text C\\
      \mathcal D_{yij,\text A}\\
      -\mathcal D_{xij,\text C}\sin\phi_\text C+\mathcal D_{zi}\cos\phi_\text C
    \end{array}
    \right].
\end{align*}
\end{Column}
