...
そのため、C側内面溝に対する傾き角$\phi_\mathrm C$についても（１つの例として）与えておく。
具体的には、以下の２点を基準として角度$\phi_\mathrm C$を取ることとする。
\begin{enumerate}
\item[a)]
...
このとき、溝位置$\kappa_p$およびボトム側の外削の高さ$h_\mathrm B$ ($h_\mathrm B > 0$)を用いると、ボトム側（$-X$側）およびトップ側（$+X$側）の外削C面の中心は、それぞれ
\begin{align*}
  \text{ボトム側：}\quad
  \left[
    \begin{array}{c}
      \displaystyle -f_\mathrm B'+\frac{h_\mathrm B}2\\
      \mathcal G_{\mathrm By}\\[4pt]
  ...
  \left[
    \begin{array}{c}
      \displaystyle f_\mathrm T'-\frac{\kappa_p}2\\
      \mathcal G_{\mathrm By}\\[4pt]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{内径}
\input{appendix_MACN/MACN_inner_diameter}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{寸法関連の決めごと}
\input{appendix_MACN/MACN_dimensional_guidelines}
...
\input{appendix_MACN/MACN_coding_guidelines}
...
\input{appendix_MACN/MACN_bundle_programs}
