...
モールドの湾曲における円の中心Oを\index{げんてん@原点}原点とした\index{ふくそすうへいめん@複素数平面}複素数平面を考える
%% footnote %%%%%%%%%%%%%%%%%%%%%
...
\end{align*}
まとめると、厚さ$\delta$の\index{スペーサ}スペーサを入れた後のトップ側の\index{ふりわけちょう@振分長}振分長$f'_\mathrm T$は、
\begin{align*}
  f'_\mathrm T
  = f_\mathrm T -l
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
トップ側とボトム側の\index{きんとうふりわけ@均等振分}振分長が同じになるとき、$\delta$は
\begin{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%
厚さ$\delta$の\index{スペーサ}スペーサを挟むと、トップ側における受板の円の中心U$_\mathrm B$は実軸方向に$\delta$だけ移動するので、
\begin{align*}
  R_\mathrm i'e^{i\alpha_{\mathrm U_\mathrm B}}
...
回転後のトップ側における受板の中心U$_\mathrm T'$とモールドの\index{わんきょくちゅうしん@湾曲中心}湾曲中心O$'$との差をとると、
\begin{align*}
...
回転後のボトム側における受板の中心U$_\mathrm B$とモールドの\index{わんきょくちゅうしん@湾曲中心}湾曲中心O$'$との差をとると、
\begin{align*}
  -\frac\delta2+\sqrt{R_\mathrm i'^2-\frac{\delta^2+(2\bar l)^2}4}\cos\psi
  -i\left\{\bar l+\sqrt{R_\mathrm i'^2-\frac{\delta^2+(2\bar l)^2}4}\sin\psi\right\}
...
厚さ$\delta$の\index{スペーサ}スペーサを挿入すると、モールドの\index{わんきょくちゅうしん@湾曲中心}湾曲中心Oは、U$_\mathrm B$を中心に$-\left(\alpha'_{\mathrm U_\mathrm B}\!-\alpha_{\mathrm U_\mathrm B}\right)$だけ回転する。
実際、
\begin{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{再振分長}
\index{スペーサ}スペーサを入れた後の（トップ側の）\index{ふりわけちょう@振分長}振分長$f'_\mathrm T$は、\pageeqref{eq:afterftUfinite}の虚部を見ればよい。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{hosoku}
改めてまとめると、厚さ$\delta$のスペーサを（トップ側に）挿入した後のトップ側の\index{ふりわけちょう@振分長}振分長$f_\mathrm T'$は、
\begin{align*}
  f_\mathrm T'
  = f_\mathrm T+\sqrt{R_\mathrm i'^2-\frac{\delta^2+(2\bar l)^2}4}\frac\delta{\sqrt{\delta^2+(2\bar l)^2}}\ .
\end{align*}
トップ側とボトム側の\index{きんとうふりわけ@均等振分}振分長が均等になるときの\index{スペーサあつ@スペーサ厚}スペーサ厚$\delta'$は、
\begin{align*}
...
これは回転の中心が、受板の点U$_\mathrm B$から\index{テーブルちゅうしん@テーブル中心}テーブル中心Pに変わることに相当する。
...
\end{align}
モールドのトップ側における\index{Cがわたんてん@C側端点}C側端点T$_\mathrm i$($R_\mathrm i$, $\alpha_{\mathrm T_\mathrm i}$)を、原点Oを中心に$\Omega$だけ回転し、さらに点P\,($\varDelta'$, $0$)を中心に$-\theta$だけ回転すると、
\begin{align}
...
\end{align}
に移動する。
同様に、トップ側における\index{Aがわたんてん@A側端点}A側端点T$_\mathrm o$($R_\mathrm o$, $\alpha_{\mathrm T_\mathrm o}$)は、
\begin{align*}
...
テーブルを傾けた後の\index{トップたんめんのわんきょくちゅうしん@トップ端面の湾曲中心}トップ端面における湾曲中心の位置T$_{R_\mathrm c}'$と、テーブル中心Pとの$X$方向の差は、
\begin{align*}
...
\index{トップたんめんのわんきょくちゅうしん@トップ端面の湾曲中心}トップ端における湾曲中心T$_{R_\mathrm c}'$と\index{トップたんめんのがいけいちゅうしん@トップ端面の外径中心}外径中心T$_\mathrm c'$との差は、以下で与えられる。
%% label{eq:TRc-Tc}
\begin{align}
  \label{eq:TRc-Tc}
...
\index{スペーサ}スペーサ取付後の\index{ボトムたんめんのわんきょくちゅうしん@ボトム端面の湾曲中心}ボトム端面における湾曲中心B$_{R_\mathrm c}'$と、テーブル中心Pとの$X$方向の差は、トップ側の場合と同様に考えて
%% footnote %%%%%%%%%%%%%%%%%%%%%
...
\index{ボトムたんめんのわんきょくちゅうしん@ボトム端面の湾曲中心}ボトム端における湾曲中心B$_{R_\mathrm c}'$と\index{ボトムたんめんのがいけいちゅうしん@ボトム端面の外径中心}外径中心B$_\mathrm c'$との差は、以下で与えられる。
%% label{eq:BRc-Bc}
\begin{align}
  \label{eq:BRc-Bc}
...
また、内面の\index{めっきまくあつ@めっき膜厚}めっき膜厚を$\mu$とし、\index{とおりしん@通り芯}通り心（\index{トップがいさくちゅうしん@トップ外削中心}トップ外削中心$\mathfrak T_\mathrm c$と\index{ボトムがいさくちゅうしん@ボトム外削中心}ボトム外削中心$\mathfrak B_\mathrm c$の差）の$X$, $Y$成分をそれぞれ$T_x$, $T_y$とする。
...
このとき、ボトム端における\index{がいけいちゅうしん@外径中心}外径中心B$_\mathrm c'$から、\index{ぼとむたんないけい@ボトム端内径}ボトム端内径$w_\mathrm B$の半分を引き、さらに\index{Aがわにくあつ@A側肉厚}A側肉厚$\tau_\mathrm B$と\index{めっきまくあつ@めっき膜厚}めっき膜厚$\mu$との差を引いたものが（おおよその）\index{がいさくAがわめん@外削A側面}外削A側面の位置$\mathfrak B_\mathrm o'$に相当する
%% footnote %%%%%%%%%%%%%%%%%%%%%
...
%% footnote %%%%%%%%%%%%%%%%%%%%%
\footnote{マシニングによって\index{きかいげんてん@機械原点}機械原点（の$X$座標）がテーブル中心Pと同じだったり異なったりする場合がある。}\relax
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{ボトムがいさくけい@ボトム外削径}ボトム側外削径の中心$\mathfrak B_\mathrm c'$の（おおよその）$X$座標は、\pageeqref{eq:spacerBc}より、
\begin{align*}
...
その原点の$X$座標（実測値）を$G_{mx}$とすると、\index{みぞはばちゅうおう@溝幅中央}溝幅中央に対するモールドの\index{Aがわがいめん@A側外面}A側外面と\index{みぞちゅうしん@溝中心}溝中心$G_{mx}$との差は、
\begin{align*}
